...
import com.probe.rpc.thrift.server.exception.ThriftServerInstantiateException;
...
public class TRegisterProcessorFactory {
	...
	public static TRegisterProcessor registerProcessor(List<ThriftServiceWrapper> serviceWrappers)
		...
		logger.info("Multiplexed processor totally owns {} service processors",
				registerProcessor.processorMetaMap.size());

		return registerProcessor;
	}

	private static void register(List<ThriftServiceWrapper> serviceWrappers, TRegisterProcessor registerProcessor)
			throws NoSuchMethodException {
		for (ThriftServiceWrapper serviceWrapper : serviceWrappers) {
			...
			if (Objects.isNull(ifaceClass)) {
				ifaceClass = Stream.of(ClassUtils.getAllInterfaces(bean))
						...
						.filter(iFace -> iFace.getDeclaringClass() != null).findFirst()
			...
			Class<TProcessor> processorClass = Stream.of(ifaceClass.getDeclaringClass().getDeclaredClasses())
					.filter(clazz -> clazz.getName().endsWith("$Processor")).filter(TProcessor.class::isAssignableFrom)
					.findFirst().map(processor -> (Class<TProcessor>) processor)
					.orElseThrow(() -> new IllegalStateException("No thrift IFace found on implementation"));
			...
			logger.info("Single processor {} register onto multiplexed processor with signature [{}]", singleProcessor,
					serviceSignature);
		}
	}
}
