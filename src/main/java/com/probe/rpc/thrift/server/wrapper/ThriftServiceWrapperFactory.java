package com.probe.rpc.thrift.server.wrapper;
...
public final class ThriftServiceWrapperFactory {
	...
	public static ThriftServiceWrapper wrapper(final String thriftServiceId, String thriftServiceName,
			Object thriftService, double version) {
		...
		Class<?> thriftServiceIface = Arrays.stream(thriftService.getClass().getInterfaces())
				.filter(iface -> iface.getName().endsWith("$Iface")).findFirst()
		...
		final String signature = String.join("$", new String[] { thriftServiceId,
				thriftServiceIface.getDeclaringClass().getName(), String.valueOf(version) });
	...
	public static ThriftServiceWrapper wrapper(final String thriftServiceId, String thriftServiceName, Class<?> type,
			Object thriftService, double version) {
