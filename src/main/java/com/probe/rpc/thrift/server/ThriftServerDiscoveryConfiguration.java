...
import com.probe.rpc.thrift.server.properties.ThriftServerDiscoveryProperties;
import com.probe.rpc.thrift.server.properties.ThriftServerHealthCheckProperties;
...
public class ThriftServerDiscoveryConfiguration {
	...
	public Consul consulClient(ThriftServerProperties thriftServerProperties) throws UnknownHostException {
		...
		registerAgentService(discoveryProperties, serviceName, serverHostAddress, servicePort, serviceId, serviceTags,
				consulClient);
		return consulClient;
	}

	private void registerAgentService(ThriftServerDiscoveryProperties discoveryProperties, String serviceName,
			String serverHostAddress, int servicePort, String serviceId, List<String> serviceTags,
			Consul consulClient) {
		AgentClient agentClient = consulClient.agentClient();
		ImmutableRegistration.Builder builder = ImmutableRegistration.builder();
		builder.id(serviceId).name(serviceName).address(serverHostAddress).port(servicePort).tags(serviceTags)
