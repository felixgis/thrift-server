package com.probe.rpc.thrift.server.annotation;
...
import com.probe.rpc.thrift.server.ProbeFactoryImportSelector;

public class ThriftServerConfigurationSelector extends ProbeFactoryImportSelector<EnableThriftServer> {
	...
	protected boolean isEnabled() {
		...
		boolean enableAutoConfiguration = StringUtils.isNotBlank(serviceId) && SERVICE_MODEL_SET.contains(serviceModel)
				&& port > 0;
		if (enableAutoConfiguration) {
			logger.info("Enable thrift server auto configuration, service id {}, service model {}, port {}", serviceId,
					serviceModel, port);
		}
